package mx.utng.smarthealthmonitor_ccdm.data.repository
...
class SyncRepository(
    ...
    suspend fun insertarLectura(lectura: LecturaFC) {
        ...
        try {
            ...
            Log.w("SYNC", "Pendiente de sync: ${e.message}")
        }
    }
...
ORDER BY created_at DESC LIMIT $1""".trimIndent(),
        ...
        Log.d("SYNC", "✅ ${response.rowCount} registros descargados de Neon")
    }
    ...
    suspend fun enviarPendientes() = withContext(Dispatchers.IO) {
        ...
        pendientes.forEach { lectura ->
            try {
                ...
                Log.d("SYNC", "✅ Sincronizado pendiente id=${lectura.id}")
            } catch (e: Exception) {
                Log.w("SYNC", "Aún sin internet: ${e.message}")
            }
        }
    }
}
